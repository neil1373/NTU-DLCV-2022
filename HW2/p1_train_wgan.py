...
def main():
    ...
    log_dir = os.path.join(workspace_dir, 'wgan_logs')
    ckpt_dir = os.path.join(workspace_dir, 'wgan_checkpoints')
    ...
    opt_D = torch.optim.RMSprop(D.parameters(), lr=config.learning_rate)
    opt_G = torch.optim.RMSprop(G.parameters(), lr=config.learning_rate)
    ...
    for e, epoch in enumerate(range(config.epoch)):
        ...
        for i in range(1000):
            torchvision.utils.save_image(f_imgs_sample[i], f'output/{i+1}.jpg')

        evaluation_paths = ['output', 'hw2_data/face/val']
        ...
        if fid_value < best_fid_value:
            ...
            face_recog_acc = face_recog.face_recog('output')
            ...
            print(f' | Epoch {e+1} | Save model with FID Score {fid_value:.6f} and Face Recognition Rate {face_recog_acc:.1f}%.')
        else:
            print(f' | Epoch {e+1} | FID Score {fid_value:.6f}')
