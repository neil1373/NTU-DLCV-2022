...
from lib.load_data import load_data, load_test_data
...
def render_viewpoints(model, render_poses, HW, Ks, ndc, render_kwargs,
    ...
    if savedir is not None and dump_images:
        ...
        for item in data['frames']:
            file_list.append(item['file_path'].split('/')[-1])
        for i in trange(len(rgbs)):
            rgb8 = utils.to8b(rgbs[i])
            filename = os.path.join(savedir, f'{file_list[i]}.png')
...
def load_everything(args, cfg, json_path):
    '''Load images / poses / camera settings / data split.
    '''
    data_dict = load_test_data(cfg.data, json_path)
...
def scene_rep_reconstruction(args, cfg, cfg_model, cfg_train, xyz_min, xyz_max, data_dict, stage, coarse_ckpt_path=None):
    ...
    if abs(cfg_model.world_bound_scale - 1) > 1e-9:
        ...
        xyz_min -= xyz_shift
        xyz_max += xyz_shift
    HW, Ks, near, far, i_train, i_val, poses, render_poses, images = [
...
if __name__=='__main__':
    ...
    # load images / poses / camera settings / data split
    data_dict = load_everything(args=args, cfg=cfg, json_path = args.json_file)
    '''
    ...
    if args.render_test:
        ...
        testsavedir = args.output_image_dir
        os.makedirs(testsavedir, exist_ok=True)
        print('All results are dumped into', testsavedir)
        rgbs, depths, bgmaps = render_viewpoints(
