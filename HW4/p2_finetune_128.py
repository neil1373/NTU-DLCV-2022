...
def main():
    ...
    ############# wandb config #############
    wandb.init(project = "DLCV_HW4_P2", name = "Finetune_Office", entity="neil1373")
    config = wandb.config
    config.epochs = 30
    config.batchsize = 64
    ...
    resnet = models.resnet50(num_classes=64)

    checkpoint_src = "resnet50_backbone_128_best.pt"
    resnet.load_state_dict(torch.load(checkpoint_src))
    checkpoint_dst = "resnet50_finetuned_128.pt"
