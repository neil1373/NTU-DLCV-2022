...
def main():
    ...
    ############# wandb config #############
    wandb.init(project = "DLCV_HW4_P2", name = "Finetune_Office_Freeze", entity="neil1373")
    config = wandb.config
    config.epochs = 30
    config.batchsize = 64
    ...
    checkpoint_src = "resnet50_backbone_256_2ndbest.pt"
    resnet.load_state_dict(torch.load(checkpoint_src))
    checkpoint_dst = "resnet50_finetuned_freeze.pt"
    ...
    for epoch in range(config.epochs):
        ...
        for i, batch in enumerate(valid_progress_bar):
            ...
            wandb.log({"Valid Finetune Accuracy": acc.item()})
